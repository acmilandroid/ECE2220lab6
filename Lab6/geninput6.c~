...
{ 
    ...
    int i, j, *rand_ids, randtimes = 0, randname = 0;
    ...
    for(i = 0; i < num_records; i++) {
        ...
        rec_ptr[i].value = 1000 * drand48(); // sets random value from [0.0, 1000.0)
        for (j = 0; j < NUMINTS; j++) {
            rec_ptr[i].a[j] = 10 * drand48(); // sets random value from [0, 9]
		}
		randtimes = 0;
		while (randtimes < 2 || randtimes > 7) { // chooses random integer from [2, 7]
			randtimes = 10 * drand48();
		}
		for (j = 0; j < randtimes; j++) { // fills in name with random ['A', 'Z']
			randname = 0;
			while (randname < 65 || randname > 90) {
				randname = 100 * drand48(); // sets random value from [0, 100)
			}
			memset(rec_ptr[i].name + j, randname, 1); // copies letter to struct
		}
		memset(rec_ptr[i].name + randtimes, '\0', 1); // ends array with '\0'
    }
